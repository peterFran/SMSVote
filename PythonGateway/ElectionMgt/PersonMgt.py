...
class PersonMgt(object):
	...
	def addPerson(self, first_name, last_name, party_types=[], election_id=None):
		c = self.conn.cursor()
		c.execute("INSERT INTO person(first_name, last_name) VALUES('%s','%s')" % (first_name, last_name))
		person_id = c.lastrowid
		self.conn.commit()
		if "CANDIDATE" in party_types and election_id is not None:
			self.makeCandidate(person_id, election_id)
	...
	def makeCandidate(self, person_id, election_id):
		c = self.conn.cursor()
		c.execute("INSERT INTO candidate(person_id, election_id) VALUES(%d,%d)" % (person_id, election_id))
		self.conn.commit()
	
	@ErrorIgnore([TypeError])
	def getPersons(self, person_id=None, party_types=[], election_id=None):
		c = self.conn.cursor()
		...
		if "VOTER" in party_types and election_id is not None:
			...
			return [self.checkCandidate(person_id, election_id)]
		else:
	...
	def checkCandidate(self, person_id, election_id):
